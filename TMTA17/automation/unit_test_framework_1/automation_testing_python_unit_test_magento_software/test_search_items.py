...
#
# 		def setUp(self) -> None: # functie care defineste instructiunile care trebuie sa se execute inainte de executarea testelor
# 				self.driver = Driver() # am facut o variabila careia i-am dat valoarea Driverului importat  # am instantiat un obiect din clasa Driver
# 				self.driver.get('https://magento.softwaretestingboard.com/') # am intrat pe url-ul din paranteza
# 				# am apelat metoda get din clasa Driver prin intermediul obiectului instantiat
# 				# self.driver.maximize_window() # am apelat metoda maximize_window din clasa Driver prin intermediul obiectului instantiat
# 				self.driver.find_element(*self.ACCEPT_COOKIES_BUTTON).click() # am cautat butonul de accept cookies prin intermediul
# 				# selectorului de tip CSS si apoi am dat click pe el
# 				# apeland metoda click() din clasa Driver apelata prin intermediul obiectului instantiat
# 				# am pus * in fata pentru ca tipul de selector si valaorea lui sunt salvate intr-un tuplu
# 				# metoda find_element cere doi parametri (tipul de selector si valoarea) si daca nu punem
# 				# * in fata, metoda va primi un singur parametru de tip tuplu
# #
...
# #
# # 		# Verify that search for a product returns proper results
...
# # 		# 		search_results = self.driver.find_elements(*self.SEARCH_RESULTS)
# # 		# 		assert len(search_results)>0,f'Expected length to be > 0, actual length: {len(search_results)}'
# # 		# 		is_product_title_correct = ""
# # 		# 		for i in range(len(search_results)):
# # 		# 				is_product_title_correct = False  # am presupus ca produsul din pozitia curenta nu are titlul corect
# # 		# 				if 'capri' not in search_results[i].text.lower(): # verificam daca asumptia noastra este corecta
# # 		# 						break # daca asumptia este corecta se intra in if, se executa break si se iese din structura repetitiva
# # 		# 											# fara a se mai executa celelalte iteratii
# # 		# 				is_product_title_correct = True # din moment ce am ajuns la instructiunea asta
# # 		# 							# inseamna ca asumptia mea initiala a fost gresita, si atunci marchez variabilea mea cu valoarea true
# # 		# 		assert is_product_title_correct == True,"Error: the product title is not correct"
# # 		#
# #
